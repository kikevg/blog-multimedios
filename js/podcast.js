...
let songs = [
    {
        name: "Frameworks del backend, para aplicaciones",
        artist: "Los miticos",
        song: "./audios/backend.mp3",
    },
    {
        name: "Bases de datos, ¿qué es? ¿Cuál es su utilidad? ¿Cómo se clasifican?",
        artist: "Los miticos",
...
const changeColor = (index) => {
    ...
    playButtons.forEach((b) => {
        if (b.getAttribute("data-song") == index) {
            b.parentElement.parentElement.children[0].classList.add(
                "text-danger"
            );
        }
    });
...
const changeIcon = () => {
    if (!isPlaying) {
        ...
        playPauseButton.children[0].innerText = "play_arrow";
    } else {
        console.log("playing");
        playPauseButton.children[0].innerText = "pause";
    }
};
